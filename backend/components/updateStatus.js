...
async function updateStatus(req,res){
    ...
    try{
        ...
        for(var i in prof_emails)
        {
            ...
            query=query.concat("_proj_list SET is_running = $1 where project_id = '");
            query = query.concat(req.body.p_id)
            query = query.concat("'")
            var db_res2 = await pool.query(query,[req.body.ans]);            

        }
        console.log("yeeeeeeeeee");
        query = "UPDATE projects SET is_running = $1 where project_id = '"
        query = query.concat(req.body.p_id)
        query = query.concat("'")

        var db_res = await pool.query(query,[req.body.ans]);
